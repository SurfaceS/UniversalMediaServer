...
public class MediaTableSubtracks extends MediaTable {
	...
	private static final String TABLE_NAME = "SUBTRACKS";
	private static final String TABLE_COL_FILEID = TABLE_NAME + ".FILEID";
	...
	protected static void insertOrUpdateSubtitleTracks(Connection connection, long fileId, DLNAMediaInfo media) throws SQLException {
		...
		String columns = "FILEID, ID, STREAM_ID, LANG, TITLE, FORMAT_TYPE, EXTERNALFILE, CHARSET ";
		...
		try (
			...
			for (DLNAMediaSubtitle subtitleTrack : media.getSubtitlesTracks()) {
				...
				try (ResultSet rs = updateStatement.executeQuery()) {
					if (rs.next()) {
						rs.updateInt("STREAM_ID", subtitleTrack.getStreamId());
						...
						insertStatement.setInt(++databaseColumnIterator, subtitleTrack.getId());
						insertStatement.setInt(++databaseColumnIterator, subtitleTrack.getStreamId());
						insertStatement.setString(++databaseColumnIterator, StringUtils.left(subtitleTrack.getLang(), SIZE_LANG));
						insertStatement.setString(++databaseColumnIterator, StringUtils.left(subtitleTrack.getSubtitlesTrackTitleFromMetadata(), SIZE_MAX));
						insertStatement.setInt(++databaseColumnIterator, subtitleTrack.getType().getStableIndex());
						if (subtitleTrack.getExternalFile() != null) {
							insertStatement.setString(++databaseColumnIterator, StringUtils.left(subtitleTrack.getExternalFile().getPath(), SIZE_EXTERNALFILE));
						} else {
							insertStatement.setString(++databaseColumnIterator, "");
						}
						insertStatement.setString(++databaseColumnIterator, StringUtils.left(subtitleTrack.getSubCharacterSet(), SIZE_MAX));
