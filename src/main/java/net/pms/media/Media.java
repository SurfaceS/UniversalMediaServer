...
public class Media implements Cloneable {
	...
	public synchronized MediaVideo getFirstVideoTrack() {
		if (hasVideo()) {
	 ...
	 * @since 1.60.0
	 */
	public List<MediaAudio> getAudioTracks() {
		return audioTracks;
	}
	 ...
	 * accordingly.
	 *
	 * @return {@code true} is this {@link Media} instance has two audio
	...
	public boolean isMuxable(Renderer renderer) {
		...
		if (
			(
				...
				(
					getWidth() > renderer.getMaxVideoWidth() ||
					getHeight() > renderer.getMaxVideoHeight()
				)
			) ||
			(
		...
		if (renderer.isBRAVIA() && getHeight() < 288) {
			muxable = false;
		}
	...
	/**
	 * Sets the {@link DLNAThumbnail} instance to use for this {@link Media} instance.
	 *
	...
	/**
	 * Determines if this {@link Media} instance has a container that is
	...
	/**
	 * Returns the {@link Format} to use if this {@link Media} instance
	 ...
	 * this {@link Media} instance represent an audio media wrapped in a
	...
	/**
	 * Returns the {@link AudioVariantInfo} to use if this {@link Media}
			...
			if (ffmpegParsing) {
				...
				synchronized (ffmpegFailureLock) {
					if (pw != null && !ffmpegFailure && !thumbOnly) {
						FFmpegParser.parse(this, pw.getResults(), input);
					}
				}
	...
	public void postParse(int type, InputFile f) {
		MediaVideo video = getFirstVideoTrack();
		String codecV = video != null ? video.getCodec() : null;
		MediaAudio audio = getFirstAudioTrack();
	...
	public Media clone() throws CloneNotSupportedException {
		Media mediaCloned = (Media) super.clone();
